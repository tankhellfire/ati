...
const lib=async(...paths)=>{
  if(paths.length===1){
    ...
    if(lib.cache[path]===undefined){
      ...
      try{
        lib.cache[path]=await Object.getPrototypeOf(async()=>{}).constructor('lib','fetch',await((await(fetch(path))).text())+';return exports')(lib,fetch)
      }catch(err){
        console.error(err)
        return
      }
...
if(typeof exports === "undefined"){let exports=lib;}